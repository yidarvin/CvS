...
def main(args):
    ...
    dataloader = create_dataloaders_dd_inference(opts.path_data,batch_size)

    # Create the model
    model = densenet101(in_chan, out_chan, pretrained=False)

    # Do the training
    inference_CvS(model, dataloader, path_save=opts.path_model, name_exp=name_exp, path_vis=path_save, verbose=True)
